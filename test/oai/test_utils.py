...
def test_config_list_from_json():
    ...
    config_list_3 = autogen.config_list_from_json(
        OAI_CONFIG_LIST, file_location=KEY_LOC, filter_dict={"model": ["gpt4", "gpt-3.5-turbo-32k"]}
    )
    assert all(config.get("model") in ["gpt4", "gpt-3.5-turbo-32k"] for config in config_list_3)
...
def test_config_list_from_dotenv(mock_os_environ, caplog):
    ...
    try:
        with os.fdopen(fd, "w+") as temp:
            ...
            # Check the default API key for gpt-3.5-turbo and gpt-3.5-turbo when model_api_key_map is None
    ...
    invalid_model_api_key_map = {
        "gpt-3.5-turbo": "INVALID_API_KEY",  # Simulate an environment var name that doesn't exist
    }
    ...
    with caplog.at_level(logging.WARNING):
        ...
        assert all(
            config["model"] != "gpt-3.5-turbo" for config in config_list
        ), "gpt-3.5-turbo configuration found, but was not expected"
        assert "API key not found or empty for model gpt-3.5-turbo" in caplog.text
